...
const AudioTranscriber = () => {
  ...
  const detectAudioLanguage = async (audioUrl) => {
    ...
    while (true) {
      const statusResp = await fetch(`https://api.assemblyai.com/v2/transcript/${data.id}`, {
        headers: {
          'authorization': '9b452376153748d6a5e309b408059e81',
          'content-type': 'application/json'
        }
  ...
  const uploadFileWithProgress = (file) => {
    return new Promise((resolve, reject) => {
      try {
        ...
        xhr.setRequestHeader('authorization', '9b452376153748d6a5e309b408059e81');
  ...
  const transcribeAudio = async () => {
    ...
    const isTranslation = languageSettings.language_code !== languageSettings.targetLanguage;
    ...
    try {
      ...
      if (detectedLang && detectedLang !== languageSettings.language_code) {
        ...
        const validModes = ['ar-ar', 'ar-en', 'en-ar', 'en-en'];
        const finalRecommended = validModes.includes(recommended) ? recommended : `${detectedLang}-${detectedLang}`;
      ...
      const checkStatus = async () => {
        try {
          ...
          if (statusData.status === 'completed') {
            if (isTranslation && statusData.translated_texts && statusData.translated_texts[languageSettings.targetLanguage]) {
              setTranscription(statusData.translated_texts[languageSettings.targetLanguage] || 'لا يوجد نص مترجم');
            } else {
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 md:p-6">
      <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        {showModeSelector ? (
          <ChosseScript 
            onChoose={(mode) => { setTranscriptionMode(mode); setShowModeSelector(false); }}
            ...
            {!transcription && audioFile && (
              ...
              >
                {isUploading ? (
                  ...
                  <>
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-6-18v4m0 0H8m4 0h4M5 7a2 2 0 100-4 2 2 0 000 4z" />
                    </svg>
